...
def inverse_rotation(rotation: torch.Tensor) -> torch.Tensor:
    """Expects a ANYx3x3 rotation matrix and returns the inverse by transposing it."""
    return rotation.transpose(-2, -1)


def inverse_intrinsics(intrinsics) -> torch.Tensor:
    """Compute the inverse of the intrinsic camera matrix.

    Note: Primary reason for not utilizing fast_inverse is that it is not supported by ONNX.

    This uses the following formula. Based on a 3x3 intrinsic matrix K:
    K = [
        [fx, skew, px],
        [0, fy, py],
        [0, 0, 1]
    ...
    The inverse K_inv is:
    K_inv = [
        [1/fx, -skew/(fx*fy), (skew*py - fy*px)/(fx*fy)],
        [0, 1/fy, -py/fy],
        [0, 0, 1]
    ]
    """
    leading_shape = intrinsics.shape[:-2]
    const_zero = torch.zeros(leading_shape).type_as(intrinsics)
    const_one = torch.ones(leading_shape).type_as(intrinsics)
    ...
    fx_fy = fx * fy  # introduced to avoid redundant computation
...
class CameraModel(nn.Module, ABC):
    def __init__(
        ...
        *,
        ...
        if is_in_onnx_export():
            # Note: torch.linalg.inv_ex is not supported by ONNX, therefore another implementation is used.
            self.rotation_inv = inverse_rotation(self.rotation)
            self.intrinsics_inv = inverse_intrinsics(self.intrinsics)
        else:
            # Due to potential scaling augmentation of the extrinsic matrix, transpose cannot be used here
    ...
    def world2cam_rot(self, sight_rays_rot):
        ...
        # Note: The explicit expand here is necessary to allow tensorrt conversion
        nr_sight_rays = sight_rays_rot.shape[-2]
        return (
            self.rotation_inv.expand(self.nr_batches, self.nr_cameras, nr_sight_rays, 3, 3)
            @ sight_rays_rot.unsqueeze(-1)
        ).squeeze(-1)
    ...
    def project_sight_rays(self, sight_rays):
        ...
        # Note: The explicit expand here is necessary to allow tensorrt conversion
        nr_sight_rays = sight_rays.shape[-2]
        return (
            self.intrinsics.expand(
                self.nr_batches,
                self.nr_cameras,
                nr_sight_rays,
                3,
...
class PinholeCameraZoomModel(PinholeCameraModel):
    ...
    def project_sight_rays(self, sight_rays):
        # Normalization factor for zoom model based on distance
        norm_dist = sight_rays[..., 2].abs()
        # Apply zoom model
        sight_rays[..., 1] = torch.asinh(sight_rays[..., 1] / norm_dist / self.mult_2) / self.mult_1 * norm_dist
        # Project camera world coordinates to image/vector pixel/array position
